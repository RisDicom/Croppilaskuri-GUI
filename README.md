...
* **Automaattinen ohjeiden generointi:**
    ...
    * Ohjeistaa mm. PTV/CTV-kopioinnissa, Boolean-operaatioissa (Union, Intersection) ja useissa Crop-operaatioissa (PTV-PTV, PTV-CTV, NT-PTV, OAR-PTV, dPTV-CTV, dPTV vs [dPTV+OAR], vNiska vs PTV/Body).
    * **Päivitetty NT-luonti:** Ohjeistaa käyttämään "Extract wall" -työkalua ja luomaan kaikki NT-rakenteet ensin, jonka jälkeen ne cropataan. Ring-rakenteen nimi on oletuksena aina **NT**.
    * **Korjattu dPTV+OAR-luonti:** Ohjeistaa käyttämään oikeaa `dPTVxx`-rakennetta (eikä `vPTVxxCrop`) Boolean AND -operaatiossa OAR:n kanssa. `dPTV+OAR` crop-marginaali on oletuksena **1.0 mm (0.1 cm)**.
...
* **Optimointikriteerien ehdotukset:**
    ...
    * Sisältää ehdotukset myös `dPTV+OAR`, `vOARcrop`, `vPTVkaikki`, `vCTVkaikki` ja `NT`-rakenteille.
...
* **Muut valinnat:**
    * Valinta `vNiska`-rakenteen luomiselle.
* **Käyttöliittymä:**
