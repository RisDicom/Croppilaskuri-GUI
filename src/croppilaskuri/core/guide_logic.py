...
def generate_guide_steps(
    ...
    oar_body_note = "Alkuperäisiä OAR:ja ja Bodya hyödynnetään suoraan, ellei alla toisin ohjeisteta."
    ...
    steps.append({"id": step_num, "title": f"A.2: Tarkista korkein annostaso ({value(str(highest_dose))} Gy)", "details_html": step2_high_details, "emoji": EMOJI_INFO})
    ...
    for ring_idx, dose_r in enumerate(ptv_doses, 1):
        ...
        steps.append({"id": step_num, "title": f"D.1.{ring_idx}: Luo {code(ring_raw_name)}", "details_html": create_ring_details, "emoji": EMOJI_RING})
    ...
    for ring_crop_idx, dose_r in enumerate(ptv_doses, 1):
        ...
        steps.append({"id": step_num, "title": f"D.2.{ring_crop_idx}: Crop {code(ring_to_crop_raw_name)} vs {code(body_name_raw)} (ulkopuolelta)", "details_html": crop_ring_body_details, "emoji": EMOJI_CROP_OUT})
