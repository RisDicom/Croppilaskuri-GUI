...
def generate_optimization_criteria_html(
    ...
    dptv_lf, dptv_uf = 0.985, 1.015
    ctv_f_lf, ctv_f_uf = 1.00, 1.02
    dptv_oar_lf, dptv_oar_uf = 0.96, 0.99
    ring_uf = 0.80
    oar_crop_uf = 0.85 if any(o.get('overlap_with_ptv_doses') for o in oars_data_list) else 0.90

    def fmt_crit(lbl: str, low: float | None = None, upp: float | None = None, nt: str | None = None) -> str:
        ln = f"<p class='crit_item'>{lbl}:"
        ind = "    "
        det: list[str] = []
        if upp is not None:
            det.append(f"{ind}Upper = <span class='value'>{upp:.2f}</span> <span class='unit'>Gy</span>")
        if low is not None:
            det.append(f"{ind}Lower = <span class='value'>{low:.2f}</span> <span class='unit'>Gy</span>")
        if det:
            ln += "<br>" + "<br>".join(det)
        if nt:
    ...
    if create_vniska:
        ...
        n_crit = n_crit.replace("</p>", "<br>    Upper = <span class='value'>???</span> <span class='unit'>Gy</span> <span class='note'>(TÄYTÄ MANUAALISESTI PROTOKOLLAN MUKAAN!)</span></p>")
    ...
    nto_list = ["<ul class='nto'>",
                "<li class='nto'>Priority: <span class='value'>~100</span></li>",
                "<li class='nto'>Distance from Target Border: <span class='value'>0.1 - 0.2</span> <span class='unit'>cm</span></li>"]
